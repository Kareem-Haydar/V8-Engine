...
namespace Core {
  ...
  #define V_DEBUG(fmt_str, ...) \
    Core::logger.Log(Core::Logger::LogLevel::Debug, TrimArgsFromFunctionName(FUNC_NAME), fmt_str, ##__VA_ARGS__)

  #define V_INFO(fmt_str, ...) \
    Core::logger.Log(Core::Logger::LogLevel::Info, TrimArgsFromFunctionName(FUNC_NAME), fmt_str, ##__VA_ARGS__)
#endif

#define V_WARNING(fmt_str, ...) \
  Core::logger.Log(Core::Logger::LogLevel::Warning, TrimArgsFromFunctionName(FUNC_NAME), fmt_str, ##__VA_ARGS__)

#define V_ERROR(fmt_str, ...) \
  Core::logger.Log(Core::Logger::LogLevel::Error, TrimArgsFromFunctionName(FUNC_NAME), fmt_str, ##__VA_ARGS__)

#define V_FATAL(fmt_str, ...) \
  Core::logger.Log(Core::Logger::LogLevel::Fatal, TrimArgsFromFunctionName(FUNC_NAME), fmt_str, ##__VA_ARGS__)
